      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <h3 className="text-xl font-bold text-foreground mb-4">{t("kurumsal.advantagesTitle")}</h3>
              <p className="text-muted-foreground leading-relaxed mb-6">{t("kurumsal.aboutText1")}</p>
              <p className="text-muted-foreground leading-relaxed">{t("kurumsal.aboutText2")}</p>
            </div>
            <div>
      ...
      <section className="py-16 md:py-24 bg-section-dark">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            <p className="text-sm font-semibold text-primary uppercase tracking-widest mb-2">{t("kurumsal.aboutLabel")}</p>
            <h2 className="text-3xl font-bold text-section-dark-foreground mb-4">{t("kurumsal.aboutCompanyName")}</h2>
            <p className="text-section-dark-foreground/70 leading-relaxed mb-8">{t("kurumsal.aboutIntro")}</p>
            <h4 className="font-bold text-section-dark-foreground mb-3">{t("kurumsal.ourExperience")}</h4>
            <ul className="space-y-2 mb-8">
              {(t("kurumsal.experienceItems", { returnObjects: true }) as string[]).map((item, i) => (
                <li key={i} className="flex items-start gap-2 text-section-dark-foreground/70">
                  <span className="mt-1.5 shrink-0 w-2 h-2 rounded-full bg-primary" />
                  {item}
                </li>
              ))}
            </ul>
            <p className="text-section-dark-foreground/70 leading-relaxed mb-4">
              <span className="font-bold text-section-dark-foreground">{t("kurumsal.ourApproachLabel")} </span>
              {t("kurumsal.ourApproachText")}
            </p>
            <p className="text-section-dark-foreground/70 leading-relaxed">
              <span className="font-bold text-section-dark-foreground">{t("kurumsal.ourCommitmentLabel")} </span>
